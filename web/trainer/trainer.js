...
function renderUnifiedMusclePieChart() {
    const container = document.getElementById('unifiedMusclePieChartContainer');
    if (!container) return;

    if (!unifiedBlockExercises || unifiedBlockExercises.length === 0) {
        container.innerHTML = `<div style="font-size: 10px; color: var(--color-text-muted); text-align: center; width: 100%; padding: 10px 0;">Selecciona ejercicios para ver el gráfico.</div>`;
        return;
    }
    ...
    const palette = [
        '#10b981', '#06b6d4', '#38bdf8', '#8b5cf6', '#f59e0b',
        '#ec4899', '#14b8a6', '#6366f1', '#f43f5e', '#a855f7'
    ];

    const sortedMuscles = Object.keys(muscleWeights).sort((a, b) => muscleWeights[b] - muscleWeights[a]);

    let currentAngle = 0;
    const slicesSvg = sortedMuscles.map((m, idx) => {
        const pct = muscleWeights[m] / totalWeight;
        ...
        return `<circle cx="45" cy="45" r="32" fill="none" stroke="${color}" stroke-width="14"
            stroke-dasharray="${dashLength.toFixed(2)} ${gapLength.toFixed(2)}"
            stroke-dashoffset="${offset.toFixed(2)}" class="pie-slice" data-muscle="${m}">
            <title>${m}: ${Math.round(pct * 100)}%</title>
        </circle>`;
    }).join('');

    const legendHtml = sortedMuscles.map((m, idx) => {
        const pct = Math.round((muscleWeights[m] / totalWeight) * 100);
        const color = palette[idx % palette.length];
        return `
            <div class="pie-legend-item">
                ...
                <span class="pie-pct-badge">${pct}%</span>
            </div>
        `;
    }).join('');
    ...
    container.innerHTML = `
        <div style="position: relative; width: 80px; height: 80px; flex-shrink: 0;">
            ...
            <div style="position: absolute; top: 0; left: 0; width: 100%; height: 100%; display: flex; flex-direction: column; align-items: center; justify-content: center; pointer-events: none;">
                <span style="font-size: 11px; font-weight: 800; color: #10b981;">${unifiedBlockExercises.length}</span>
                <span style="font-size: 7.5px; color: var(--color-text-muted);">Ejercicios</span>
            </div>
        </div>
