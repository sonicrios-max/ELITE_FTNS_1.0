...
function selectExerciseCategoryFilter(category) {
    ...
    // Highlight in global muscle map SVG
    const paths = document.querySelectorAll('#globalMuscleMapContainer .muscle-group-path');
    paths.forEach(p => {
        const m = p.getAttribute('data-muscle');
        if (category !== 'Todos' && (m === category || (category === 'Zona Core' && (m === 'Abdominales' || m === 'Oblicuos' || m === 'Espalda Baja')))) {
    ...
    if (label) {
        if (category === 'Todos') {
            ...
            label.textContent = `Músculo: ${category}`;
...
function getUniqueMusclesList() {
    const allMuscles = new Set();
    globalExercisesCache.forEach(ex => {
        if (ex.primary_muscle) {
            allMuscles.add(ex.primary_muscle.trim());
        }
...
function renderExerciseCategoryFilters() {
    ...
    // Extract unique muscles (primary + secondary) dynamically from the loaded exercises
    const muscles = getUniqueMusclesList();
    
    const categories = ['Todos', ...muscles];
...
function renderExercisesTable() {
    ...
    let filtered = globalExercisesCache.filter(ex => {
        ...
        const catMatch = activeExerciseCategoryFilter === 'Todos' || 
            (ex.primary_muscle && ex.primary_muscle.toLowerCase() === activeExerciseCategoryFilter.toLowerCase()) ||
            (ex.secondary_muscles && ex.secondary_muscles.split(',').map(m => m.trim().toLowerCase()).includes(activeExerciseCategoryFilter.toLowerCase()));
...
function openBlockModal() { 
    ...
    // Reset view to Front
    unifiedMuscleViewFront = true;
    const frontView = document.getElementById('unifiedFrontView');
    const backView = document.getElementById('unifiedBackView');
    if (frontView && backView) {
        frontView.style.display = 'flex';
        backView.style.display = 'none';
    }
    const btnToggle = document.getElementById('btnToggleUnifiedView');
    if (btnToggle) {
        btnToggle.innerHTML = '<i class="fa-solid fa-arrows-rotate"></i> Frente / Espalda';
    }
...
function renderUnifiedCatalog() {
    ...
    if (muscle) {
        filtered = filtered.filter(ex => 
            (ex.primary_muscle && ex.primary_muscle.toLowerCase() === muscle.toLowerCase()) ||
            (ex.secondary_muscles && ex.secondary_muscles.split(',').map(m => m.trim().toLowerCase()).includes(muscle.toLowerCase()))
        );
    }
...
// editingBlockId already declared above
...
function selectMuscleFromMap(muscle) {
    // If we clicked the currently active muscle, toggle it off (select 'Todos')
    if (activeExerciseCategoryFilter === muscle) {
        clearMuscleMapSelection();
        return;
    }
    
    // Highlight matching paths in global map
    const paths = document.querySelectorAll('#globalMuscleMapContainer .muscle-group-path');
    paths.forEach(p => {
        const m = p.getAttribute('data-muscle');
    ...
    // Set filter category and refresh table
...
// Block Creator (Unified Wizard) Muscle Map functions
function toggleUnifiedMuscleView() {
    ...
    if (btn) {
        btn.innerHTML = unifiedMuscleViewFront ? '<i class="fa-solid fa-arrows-rotate"></i> Frente' : '<i class="fa-solid fa-arrows-rotate"></i> Espalda';
    }
}
...
function selectUnifiedMuscleFromMap(muscle) {
    ...
    // If clicked active, toggle off
    if (select.value === muscle) {
        clearUnifiedMuscleSelection();
        return;
    }
    
    // Highlight paths in the wizard SVGs
